...
export function useUpdateTrack() {
    const queryClient = useQueryClient();
    return useMutation<UpdateTrackResponse, Error, { slug: string; data: UpdateTrackRequest }>({
        mutationFn: async ({ slug, data }): Promise<UpdateTrackResponse> => {
            return await tracksApi.updateTrack(slug, data);
        },
        onSuccess: (res, variables) => {
            queryClient.invalidateQueries({ queryKey: queryKeys.tracks.all });
            queryClient.invalidateQueries({ queryKey: queryKeys.tracks.detail(variables.slug) });
